...
public class calculator {
    ...
    private ArrayList infixToPostfix(ArrayList constant) {
        ...
        for (Object object : constant) {
            if (level.containsKey(object)) {
                if (stack.isEmpty()) {
                    stack.push(object);
                } else {
                    if(Double.parseDouble(level.get(stack.peek()).toString()) >=
                            Double.parseDouble(level.get(object).toString())) {
    ...
    private Double postFixEval(ArrayList expr) {
        ...
        resultN = (Double) numberStack.pop();
